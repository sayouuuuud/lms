...
function LectureCard({ lecture, index }: { lecture: Lecture; index: number }) {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-[1.75rem] border border-navy/10 bg-white shadow-sm ring-1 ring-transparent transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-navy/10 hover:ring-emerald-brand/30">
      {/* lecture image with price + number overlays */}
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-gradient-to-br from-cream to-navy/5">
        <Image
        ...
        />
        {/* number chip */}
        <span className="absolute right-4 top-4 grid size-10 place-items-center rounded-xl border border-navy/10 bg-white/90 font-mono text-sm font-bold text-navy backdrop-blur">
          {String(index + 1).padStart(2, '0')}
        </span>
        {/* badge */}
        {lecture.badge && (
          <span className="absolute left-4 top-4 rounded-full bg-gold px-3 py-1 text-xs font-bold text-navy shadow-lg">
            {lecture.badge}
          </span>
        )}
        {/* price tag */}
        <span className="absolute bottom-4 left-4 inline-flex items-baseline gap-1.5 rounded-2xl bg-navy px-4 py-2 shadow-lg shadow-navy/30">
          {lecture.oldPrice && (
          ...
          <span className="text-xs font-bold text-gold">ج.م</span>
        </span>
      </div>

      {/* body */}
      <div className="flex flex-1 flex-col p-6">
        <h3 className="font-heading text-xl font-bold text-navy">{lecture.title}</h3>
        <p className="mt-2 text-pretty text-sm leading-relaxed text-navy-soft">
          {lecture.description}
        </p>
        ...
        <button
          ...
          className="mt-5 flex w-full items-center justify-between rounded-2xl border border-navy/10 bg-cream/60 px-4 py-3 text-sm font-bold text-navy transition-colors hover:bg-cream"
        >
        ...
        <Link
          ...
          className="mt-4 flex items-center justify-center gap-2 rounded-full bg-navy px-6 py-3.5 text-sm font-bold text-cream transition-colors hover:bg-navy-deep"
        >
      ...
      {open && (
        <div className="absolute inset-0 z-10 flex flex-col bg-white">
          {/* panel header */}
          <div className="flex shrink-0 items-center justify-between gap-3 border-b border-navy/10 bg-cream/60 px-5 py-4">
            <div>
              <span className="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-deep">
                <Layers className="size-3.5" />
                محتوى المحاضرة
              </span>
              <h3 className="mt-0.5 font-heading text-lg font-bold text-navy">{lecture.title}</h3>
            </div>
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="grid size-9 shrink-0 place-items-center rounded-full bg-white text-navy shadow-sm transition-colors hover:bg-navy hover:text-cream"
              aria-label="إغلاق"
            >
              <X className="size-4" />
            </button>
          ...
          <ul className="flex-1 space-y-1 overflow-y-auto p-3">
            {lecture.lessons.map((lesson, li) => (
              <li
                key={lesson.id}
                className="flex items-center justify-between gap-3 rounded-xl px-3 py-2.5 transition-colors hover:bg-cream/60"
              >
                ...
                <span className="shrink-0 font-mono text-xs text-navy-soft">
                  {lesson.duration}
                </span>
              </li>
            ))}
          ...
          {/* panel footer CTA */}
          <div className="shrink-0 border-t border-navy/10 p-3">
            <Link
              href="/auth?mode=register"
              className="flex items-center justify-center gap-2 rounded-full bg-navy px-6 py-3 text-sm font-bold text-cream transition-colors hover:bg-navy-deep"
            >
      ...
      <section className="relative overflow-hidden bg-navy">
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-16 pt-28 md:px-8 md:pb-24 md:pt-32">
          ...
          <nav className="flex flex-wrap items-center gap-2 text-sm font-semibold text-cream/60">
            <Link href="/#stages" className="transition-colors hover:text-gold">
              المراحل
            </Link>
            <ArrowRight className="size-3.5" />
            <Link href={`/stages/${stage.id}`} className="transition-colors hover:text-gold">
              {stage.title}
            </Link>
            <ArrowRight className="size-3.5" />
      ...
      <section className="relative mx-auto max-w-6xl px-5 py-12 md:px-8 md:py-16">
        ...
        <div className="mt-12 flex justify-center">
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-full border border-navy/15 px-6 py-3 text-sm font-bold text-navy transition-colors hover:bg-navy/5"
          >
