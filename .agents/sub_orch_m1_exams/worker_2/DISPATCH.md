## 2026-08-20T19:50:16Z

You are the Implementation Worker for Milestone 1 (M1: Exams Edge Cases) - Worker 2 (Replacement).
Working directory: d:/Workspace/LMS/.agents/sub_orch_m1_exams/worker_2
...
Commands to Execute & Verify (Always prefix with `cmd /c`):
- `cmd /c npx prisma generate`
- Run each test script:
  `cmd /c node --env-file-if-exists=.env scripts/test_exam_resume.mjs`
  `cmd /c node --env-file-if-exists=.env scripts/test_exam_server_timer.mjs`
  `cmd /c node --env-file-if-exists=.env scripts/test_exam_double_submit.mjs`
  `cmd /c node --env-file-if-exists=.env scripts/test_exam_snapshot_integrity.mjs`
- Run typecheck: `cmd /c npx tsc --noEmit`
