  ...
  return (
    <main className="min-h-screen bg-background px-4 py-14 sm:px-6 md:px-10" dir="rtl">
      <div className="mx-auto max-w-6xl">
        <Link href="/subscriptions" className="inline-flex items-center gap-2 text-sm font-bold text-purple-deep hover:underline dark:text-teal-glow"><ArrowRight className="size-4" />العودة إلى الخطط</Link>
        ...
        <section className="mt-7 rounded-[2rem] border border-border bg-card p-6 shadow-sm sm:p-8 dark:border-border dark:bg-card"><h2 className="font-heading text-2xl font-extrabold text-foreground">ماذا يشمل الاشتراك؟</h2><p className="mt-2 text-sm leading-7 text-foreground-soft dark:text-muted-foreground">الوصول الذي تمنحه هذه الخطة ينتهي بانتهاء مدتها، بينما أي كورس اشتريته بشكل منفرد يظل مملوكًا لك بشكل دائم.</p><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{plan.scopes.length ? plan.scopes.map((scope, index) => <div key={`${scope.scopeType}-${scope.scopeId ?? index}`} className="flex items-center gap-3 rounded-2xl bg-muted p-4 text-sm font-bold text-foreground"><CheckCircle2 className="size-5 shrink-0 text-purple-deep dark:text-teal-glow" />{scopeText(scope.scopeType)}</div>) : <div className="rounded-2xl bg-muted p-4 text-sm font-bold text-foreground">كل المحتوى المنشور المتاح ضمن المنصة</div>}</div></section>
      </div>
    </main>
  )
}
