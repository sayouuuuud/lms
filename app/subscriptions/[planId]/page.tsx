...
import { getPublicSubscriptionPlan } from '@/lib/subscription-public'
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-7 grid overflow-hidden rounded-[2rem] border border-border bg-card shadow-xl lg:grid-cols-[0.85fr_1.15fr] dark:border-border dark:bg-card">
          ...
          <div className="p-6 sm:p-9">
            ...
            <div className="mt-7 flex flex-col gap-4 rounded-2xl border border-gold/30 bg-gold/10 p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="text-sm font-bold text-foreground">سعر الاشتراك</div><div className="mt-1 font-heading text-3xl font-extrabold text-foreground">{formatMoney(plan.price)} <small className="text-sm font-normal">ج.م</small></div></div><Link href={`/auth/register?planId=${encodeURIComponent(plan.id)}`} className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-7 py-3.5 text-sm font-bold text-primary-foreground transition hover:bg-primary-deep">ابدأ الاشتراك <ArrowRight className="size-4 -rotate-180" /></Link></div>
          </div>
        </div>
