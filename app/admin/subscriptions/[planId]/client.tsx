        ...
        <div className="space-y-6">
          ...
          <Card><CardHeader><CardTitle>النطاق والمحتوى المشمول</CardTitle></CardHeader><CardContent className="space-y-4">
            ...
            {plan.scopeMode === 'all_released' ? <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800">الخطة تفتح كل المحتوى المنشور. لا تحتاج إلى اختيار عناصر فردية.</div> : <div className="space-y-5">
              ...
              <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><h3 className="font-semibold text-emerald-950">ملخص المحتوى المختار</h3><p className="text-xs text-emerald-800">هذه هي الصفوف المسطحة التي ستُحفظ للخطة عند الضغط على الحفظ.</p></div><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-900">{scopes.filter((scope) => scope.scope_type !== 'all_released').length} اختيار</span></div><div className="flex flex-wrap gap-2">{scopes.filter((scope) => scope.scope_id).map((scope) => <span key={`${scope.scope_type}-${scope.scope_id}`} className="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-white px-2.5 py-1 text-xs text-emerald-950"><span>{scopeTypeLabels[scope.scope_type] ?? scope.scope_type}: {getScopeLabel(scope) ?? scope.scope_id}</span><button type="button" onClick={() => removeScope(scope.scope_type, scope.scope_id!)} className="font-bold text-emerald-700 hover:text-rose-700" aria-label="حذف الاختيار">×</button></span>)}</div>{scopes.filter((scope) => scope.scope_id).length === 0 && <p className="text-sm text-emerald-800">لم يتم اختيار محتوى بعد.</p>}</div>
            </div>}
          </CardContent></Card>
        </div>
