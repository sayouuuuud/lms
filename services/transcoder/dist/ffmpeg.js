...
function buildFfmpegArgs(opts) {
    ...
    args.push('-c:v', 'libx264', '-preset:v', 'veryfast', '-crf:v', '23', // جودة ممتازة مقاربة للأصل بضغط ذكي
    '-profile:v', 'main', '-level:v', '4.1', '-sc_threshold:v', '0', '-g:v', '48', '-keyint_min:v', '48', '-c:a', 'aac', '-b:a', '128k', '-ar:a', '44100', '-ac:a', '2', '-f', 'hls', '-hls_time', String(segmentDurationSec), '-hls_playlist_type', 'vod', '-hls_segment_type', 'mpegts', '-hls_segment_filename', path.join(segDir, 'seg%05d.ts'), '-hls_flags', 'independent_segments', path.join(segDir, 'index.m3u8'));
    return args;
}
// ---------------------------------------------------------------
// runFfmpeg: ينفّذ العملية مع progress tracking
// ---------------------------------------------------------------
async function runFfmpeg(args, totalSeconds, onProgress) {
...
// ---------------------------------------------------------------
async function writeMasterManifest(outputDir) {
    const lines = [
        '#EXTM3U',
        '#EXT-X-VERSION:3',
        '',
